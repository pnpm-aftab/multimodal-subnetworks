...
# CRITICAL CHANGE: Fixed seed for reproducibility instead of random seed
# SEED = random.randint(0, 9999)  # OLD - not reproducible
...
class CustomRunner(dl.Runner):
    def __init__(
        ...
        self.fixed_seed = fixed_seed  # NEW: Store fixed seed
    ...
    @property
    def seed(self) -> int:
        """Use fixed seed for reproducibility instead of random seed."""
    ...
    def get_loaders(self, stage=None):
        ...
        if self.masked:
            ...
            # CRITICAL CHANGE: Use fixed seed for reproducible SNIP sampling
        ...
        train_sampler = (
            DBBatchSampler(train_dataset, batch_size=self.num_volumes, seed=self.fixed_seed)  # NEW: Use fixed seed
        ...
        train_dataloader = BatchPrefetchLoaderWrapper(
            DataLoader(
                ...
                prefetch_factor=self.prefetches,
        ...
        valid_sampler = (
            DBBatchSampler(valid_dataset, batch_size=self.num_volumes, seed=self.fixed_seed)  # NEW: Use fixed seed
            if self.engine.is_ddp
            else DBBatchSampler(
                valid_dataset, batch_size=self.num_volumes, seed=self.fixed_seed  # NEW: Use fixed seed
            )
        )
        ...
        test_sampler = (
            DBBatchSampler(test_dataset, batch_size=self.num_volumes, seed=self.fixed_seed)  # NEW: Use fixed seed
            if self.engine.is_ddp
            else DBBatchSampler(
                test_dataset, batch_size=self.num_volumes, seed=self.fixed_seed  # NEW: Use fixed seed
            )
        )
...
def main(cfg: DictConfig):
    # CRITICAL: Set fixed seed globally at the start for reproducibility
    fixed_seed = cfg.experiment.get("fixed_seed", 1997)
    utils.set_global_seed(fixed_seed)
    setup_distributed_port(seed=fixed_seed)
    print(f"[FIXED SEED MODE] Using reproducible seed: {fixed_seed}")
    ...
    fixed_seed = cfg.experiment.get("fixed_seed", 1997)  # NEW: Get fixed_seed from config
    ...
    for fold_idx in range(cfg.experiment.cv_folds):
        ...
        runner = CustomRunner(
            ...
            hparams=hparams,
            fixed_seed=fixed_seed,  # NEW: Pass fixed seed to runner
        )
        runner.run()
        del runner
